...
class FileExplorer(object):
    ...
    def __init__(self, root, window, font_size=14, row_height=14):
        ...
        self.btn_resync = tk.Button(self.lower_frame_left, text="Donate", width=10, command=self.btn_donate_click)
        self.btn_resync.pack(anchor="w")

        self.lower_frame_right = tk.Frame(self.lower_frame)
        self.lower_frame_right.pack(side=tk.LEFT, expand=True, fill=tk.X)

        self.log_widget = tk.scrolledtext.ScrolledText(self.lower_frame_right, height=5)
    ...
    def btn_donate_click(self):
        webbrowser.open_new("https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=9B8J938QN53T2")
